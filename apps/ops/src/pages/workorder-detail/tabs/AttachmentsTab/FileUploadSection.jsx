// workorder-details/tabs/AttachmentsTab/FileUploadSection.jsx
...
/**
 * Reusable upload section for a single work order attachment category.
 *
 * Props:
 *  - category:  "pre_work" | "vendor_before" | "vendor_after" | "attachment"
 *  - title, subtitle: section header text
 *  - variant:   "grid" (image thumbnails) | "list" (file rows)
 *  - accept:    input accept string (default any)
 *  - readOnly:  hide the dropzone (view-only, e.g. vendor-uploaded)
 */
export default function FileUploadSection({
  ...
  const uploadFiles = async (fileList) => {
    ...
    try {
      ...
      console.error(`Error uploading ${category}:`, e);
    } finally {
      setUploading(false);
    }
  };
  ...
  const deleteItem = async (itemId) => {
    ...
    try {
      await axios.delete(`/api/workorders/${id}/attachments/${itemId}`);
  ...
  return (
    ...
    >
      ...
      <Box sx={{ p: 2 }}>
        {/* Compact dropzone (hidden when readOnly) */}
        {!readOnly && (
          <Box
