...
async function migrateVendorNotes() {
  ...
  let stats = { notes: 0, tags: 0, skippedVendors: 0, skippedTags: 0 };

  for (const vendor of vendorsWithNotes) {
    const vendorSqlId = vendorIdMap.get(vendor.id);
    if (!vendorSqlId) {
      console.warn(
        `Skipping vendor ${vendor.name} (sarlaccId ${vendor.id}) — not in SQL`,
      );
